...
const { data: projects } = await useAsyncData('project-list', () => queryCollection('projects').all())
...
const currentIndex = projects.value?.findIndex(p => p.path === currentProject.value?.path) ?? -1
